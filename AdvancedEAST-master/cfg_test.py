...
# data_dir = 'Test_train/'
data_dir = 'Test_train/'
origin_image_dir_name = 'imageg_8/'
origin_txt_dir_name = 'txt_8/'
...
if not os.path.exists('model_test'):
    os.mkdir('model_test')
if not os.path.exists('saved_model_test'):
    os.mkdir('saved_model_test')

model_weights_path = 'model_test/weights_%s.{epoch:03d}-{val_loss:.3f}.h5' \
                     % train_task_id
saved_model_file_path = 'saved_model_test/east_model_%s.h5' % train_task_id
saved_model_weights_file_path = 'saved_model_test/east_model_weights_%s.h5'\
                                % train_task_id
# 目标点像素阈值
pixel_threshold = 0.9
# 边像素阈值
